# ==============================================================================
# Script: Bat Heart scRNA-seq Full Pipeline (Harmony Integration & Visualization)
# Description: Merging, QC Filtering, Harmony Integration, and Publishable Plots
# ==============================================================================
...
# ==============================================================================
# PART 1: HARMONY INTEGRATION
# ==============================================================================
x.merge <- qs::qread("bat-heart-merged.qs")
Idents(x.merge) <- x.merge$sample

# Apply strict QC filters
x.merge <- subset(x.merge, subset = nFeature_RNA > 400 & nFeature_RNA < 4000 & 
                                   nCount_RNA < 10000 & percent.mt < 5)

# Split by species for independent SCTransform processing
obj_list <- SplitObject(x.merge, split.by = 'species')
...
# Run SCTransform on the merged object and set variable features
x.merge <- SCTransform(x.merge, verbose = FALSE)
DefaultAssay(x.merge) <- "SCT"
VariableFeatures(x.merge) <- features

# Run PCA and Harmony
x <- RunPCA(x.merge, assay = "SCT", npcs = 50, verbose = FALSE)
x <- RunHarmony(x, group.by.vars = "species", reduction = "pca", assay.use = "SCT", reduction.save = "harmony")

# Dimensionality Reduction using Harmony
x <- RunUMAP(x, reduction = "harmony", dims = 1:20, verbose = FALSE)
x <- FindNeighbors(x, reduction = "harmony", dims = 1:20, verbose = FALSE)

# ==============================================================================
# PART 2: CLUSTERING & ANNOTATION
# ==============================================================================
cat("Starting Part 3: Clustering and Annotation...\n")

x <- FindClusters(x, verbose = FALSE, resolution = 0.1)

# Normalize RNA assay for marker visualization
DefaultAssay(x) <- "RNA"
x <- NormalizeData(x)
x <- ScaleData(x, features = rownames(x))

# Apply manual annotations based on your previous mapping
cell.type2 <- c("Cardiomyocyte", "Fibroblast", "Lym-Endothelia", "Schwann", 
                "Vas-Endothelia", "Macrophage", "Pericyte",
                "T", "Adipocyte", "Mesothelia", "Leukocyte")
...
qs::qsave(x, "bat-heart-harmony-annotated.qs")

# ==============================================================================
# PART 3: COLOR PALETTE SETUP
# ==============================================================================
# Lock factor levels for consistent plotting
...
x$cell.type2 <- factor(x$cell.type2, levels = original_clusters)
Idents(x) <- x$cell.type2

# Numeric IDs for clean UMAP labels
...
names(my_colors_named) <- original_clusters 
...
# ==============================================================================
cat("Starting Part 5: Plotting UMAPs...\n")
...
p_global <- DimPlot(x, reduction = "umap", group.by = "cell.type2", cols = my_colors_named, raster = TRUE) + 
  ...
  theme_classic() +
  ggtitle("Bat Heart scRNA-seq Atlas (Harmony)")

pdf("bat-heart-harmony-global-umap.pdf", width = 10, height = 8)
print(p_global)
dev.off()

# B. Split UMAPs
pdf("bat-heart-harmony-split-species.pdf", width = 20, height = 5)
DimPlot(x, split.by = 'species', cols = my_colors_named, label = TRUE, raster = TRUE) + 
  theme_classic() + NoLegend()
dev.off()

pdf("bat-heart-harmony-split-sample.pdf", width = 20, height = 10)
...
# ==============================================================================
cat("Starting Part 6: Marker Analysis...\n")

# Calculate All Markers
x.markers <- FindAllMarkers(x, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(x.markers, "bat-heart-harmony-markers.csv", row.names = FALSE)

# Plot Canonical Markers
canonical_markers <- c(
  "RYR2", "ACTN2",      # Cardiomyocyte-1
  "COX6A2", "ACTC1",    # Cardiomyocyte-2
  "FLT1", "ADGRL4",     # Endothelia-1
  "LYVE1", "CCL21",     # Endothelia-2
  "LUM", "DCN",         # Fibroblast
...
pdf("bat-heart-harmony-canonical-markers-dotplot.pdf", height = 6, width = 9)
...
# ==============================================================================
cat("Starting Part 7: Composition Barplot...\n")

comp_data <- x@meta.data %>%
...
p_comp <- ggplot(comp_data, aes(x = sample, y = CellCount, fill = cell.type2)) +
  ...
  theme_bw() +
  labs(title = "Cell Type Composition per Sample (Harmony)",
       x = "Sample",
       y = "Number of Cells") +
  theme(
...
pdf("bat-heart-harmony-cell-composition-barplot.pdf", width = 8, height = 6)
print(p_comp)
dev.off()
